...
void Get_Password(char *password,int size)
{
    int counter=0;
        ...
        while(counter<size)
        {
            ...
            {}
			password[counter] = USART_ReceiveData(USART2);
            Uart_Put_Char(USART2,password[counter]);
            counter++;
        }
}
